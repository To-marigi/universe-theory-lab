"""Scalar expansion for the smallest GC endpoint blocks.

The 1,529 same-endpoint GC pairs are split by endpoint.  This gate selects
only endpoints with exactly three paths, expands each cached path once, then
checks all 33 pairs in those 11 blocks.  Larger endpoint blocks are not
included and are not inferred from this result.
"""
...
RESULT_PATH = "results/v0.4.2_721_gc_small_blocks.json"
SCHEMA = "final-theory-v042-721-gc-small-blocks-v1"
VERDICT = "V042_721_GC_11_THREE_PATH_BLOCKS_SCALAR_COMPILED_OPEN"
...
def compile_gc_small_blocks(root: Path) -> dict[str, Any]:
    root = root.resolve()
    ...
    if len(selected_endpoints) != 11:
        raise AssertionError("three-path endpoint block count changed")
    selected_paths = {
        path["path_id"]: path
    ...
    if len(selected_pairs) != 33:
        raise AssertionError("three-path endpoint pair count changed")
    ...
    payload: dict[str, Any] = {
        ...
        "scope": {
            "selected_endpoint_rule": "exactly three paths",
        ...
        "claim_boundary": [
            "Only the 11 endpoint blocks with exactly three paths are scalar-expanded.",
            "Path matrices are cached once per selected path before pair differences.",
            "The 1,496 remaining GC pairs are not inferred from these blocks.",
...
def write_gc_small_blocks(root: Path) -> Path:
    path = root / RESULT_PATH
    path.write_text(
        json.dumps(compile_gc_small_blocks(root), ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
        newline="\n",
    )
    return path
...
if __name__ == "__main__":
    print(write_gc_small_blocks(Path(__file__).resolve().parents[3]))
