"""Scalar expansion for the four-path GC endpoint blocks.

Successor to the three-path small-blocks gate.  The 1,529 same-endpoint GC
pairs are split by endpoint.  This gate selects only endpoints with exactly
four paths, expands each cached path once, then checks all 54 pairs in those
9 blocks.  Endpoints with three or fewer/more paths are not included and are
...
from universe_lab.final_theory.source_native_721_gc_small_blocks_v042 import (
    RESULT_PATH as PREDECESSOR_RESULT_PATH,
)
...
RESULT_PATH = "results/v0.4.2_721_gc_four_path_blocks.json"
SCHEMA = "final-theory-v042-721-gc-four-path-blocks-v1"
VERDICT = "V042_721_GC_9_FOUR_PATH_BLOCKS_SCALAR_COMPILED_OPEN"
...
def compile_gc_four_path_blocks(root: Path) -> dict[str, Any]:
    root = root.resolve()
    ...
    selected_endpoints = sorted(
        ...
        if len(paths) == SELECTED_PATH_COUNT
    )
    if len(selected_endpoints) != 9:
        raise AssertionError("four-path endpoint block count changed")
    selected_paths = {
        path["path_id"]: path
    ...
    if len(selected_pairs) != 54:
        raise AssertionError("four-path endpoint pair count changed")
    ...
    if len(selected_basis) != 27:
        raise AssertionError("selected endpoint basis count changed")
    ...
    predecessor_expanded = json.loads(
        (root / PREDECESSOR_RESULT_PATH).read_text(encoding="utf-8")
    )["global_gc_boundary"]["expanded_here"]
    combined_expanded = predecessor_expanded + len(records)
    payload: dict[str, Any] = {
        "schema_version": SCHEMA,
        "profile": "fixed_vector_GC__strong_MSR",
        "scope": {
            "selected_endpoint_rule": "exactly four paths",
        ...
        "predecessor_gate_sha256": _sha256(root / PREDECESSOR_RESULT_PATH),
        "paths": {
        ...
        "global_gc_boundary": {
            ...
            "expanded_cumulative_with_predecessor": combined_expanded,
            "remaining_pair_count": all_pair_count - combined_expanded,
        },
        "solver_status": {
        ...
        "claim_boundary": [
            "Only the 9 endpoint blocks with exactly four paths are scalar-expanded.",
            "Path matrices are cached once per selected path before pair differences.",
            "The remaining GC pairs outside this and the predecessor gate are not inferred.",
...
def write_gc_four_path_blocks(root: Path) -> Path:
    path = root / RESULT_PATH
    path.write_text(
        json.dumps(compile_gc_four_path_blocks(root), ensure_ascii=False, indent=2)
        + "\n",
        encoding="utf-8",
        newline="\n",
    )
...
if __name__ == "__main__":
    print(write_gc_four_path_blocks(Path(__file__).resolve().parents[3]))
